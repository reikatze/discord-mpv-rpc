...
local function score_multi_result(result, query_title, year, prefer_tv)
    ...
    local similarity = best_title_similarity(query_title, result)
...
local function new_resolver(callbacks)
    ...
    local function best_alias_match(query_title, result, lookup_token)
        local aliases, outcome = alternative_titles(result, lookup_token)
        if outcome == 'cancelled' then return 0, nil, true end
        if not aliases or #aliases == 0 then return 0, nil, false end
        ...
        for i = 1, #aliases do
            ...
            if similarity > best_similarity then
                best_similarity, best_title = similarity, aliases[i]
            end
        end
        ...
        for i = 1, #query_titles do
            ...
            if use_aliases then
                ...
                alias_similarity, alias_title, cancelled =
                    best_alias_match(query_title, result, lookup_token)
                if cancelled then return nil, nil, nil, nil, true end
    ...
    local function choose_from_pool(
        ...
        local best, second = scored[1], scored[2]
    ...
    local function resolve_aliases_tiered(pool, query_titles, year, is_tv, lookup_token)
        local ranked = {}
        for i = 1, #pool do
            if lookup_cancelled(lookup_token) then
